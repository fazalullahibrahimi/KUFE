...
const CommitteeMemberManagement = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch users
        const usersResponse = await fetch("http://localhost:4400/api/v1/user/justNameOfComiteeMembers", {
          headers,
        })
        ...
        try {
          const committeeMembersResponse = await fetch("http://127.0.0.1:4400/api/v1/committee-members/", {
            headers,
          })
          ...
          } else if (committeeMembersData && committeeMembersData.data && Array.isArray(committeeMembersData.data)) {
            setCommitteeMembers(committeeMembersData.data)
          } else {
            console.warn("Committee members data structure is not as expected:", committeeMembersData)
            setCommitteeMembers([])
          }
        } catch (error) {
  ...
  const validateForm = () => {
    ...
    if (!formData.academicRank) errors.academicRank = "Academic rank is required"
    if (!formData.committeePosition) errors.committeePosition = "Committee position is required"
  ...
  const handleAddMember = async (e) => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:4400/api/v1/committee-members/", {
        method: "POST",
      ...
      if (response.ok) {
        ...
        const refreshResponse = await fetch("http://127.0.0.1:4400/api/v1/committee-members/", {
          headers: createHeaders(),
        })
        ...
        } else if (refreshData && refreshData.data && Array.isArray(refreshData.data)) {
  ...
  const handleEditMember = async (e) => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:4400/api/v1/committee-members/${selectedMember._id}`, {
        method: "PATCH",
        headers: createHeaders(),
        body: JSON.stringify({
          userId: formData.userId,
  ...
  const openEditModal = (member) => {
    ...
    let departmentId = ""
    if (member.department && typeof member.department === "object" && member.department._id) {
      departmentId = member.department._id
    } else if (typeof member.department === "string") {
      departmentId = member.department
    }

    console.log("Setting form data with userId:", userId, "and departmentId:", departmentId)
  ...
  const filteredMembers = Array.isArray(committeeMembers)
    ? committeeMembers.filter((member) => {
        ...
        return (
          (member.userId?.fullName || "").toLowerCase().includes(searchString) ||
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-800 mb-2">{t("Committee Member Management")}</h1>
        <p className="text-gray-600">{t("Manage committee members, their roles, and contact information")}</p>
      </div>
        ...
        <div className="bg-white p-8 rounded-lg shadow text-center">
          <p>
            {searchTerm
              ...
              : t("No committee members found. Add your first committee member.")}
          </p>
        </div>
      )}

      {/* Add Committee Member Modal */}
      <Modal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} title={t("Add Committee Member")}>
        <form onSubmit={handleAddMember}>
          ...
          <div className="flex justify-end gap-3 mt-6">
            ...
            <button type="submit" className="px-4 py-2 bg-[#004B87] text-white rounded-md hover:bg-[#003b6a]">
              {t("Add Committee Member")}
            </button>
          </div>
        </form>
      ...
      <Modal isOpen={isViewModalOpen} onClose={() => setIsViewModalOpen(false)} title={t("Committee Member Details")}>
        {selectedMember && (
          <div className="space-y-4">
            <div className="bg-gray-50 p-4 rounded-lg">
              <h3 className="font-medium text-lg mb-2">{selectedMember.userId?.fullName || "N/A"}</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                ...
                <div>
                  <p className="text-sm text-gray-500">{t("Committee Position")}</p>
      ...
      <Modal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} title={t("Confirm Deletion")}>
        <div className="p-4">
          <p className="mb-4">
            {t("Are you sure you want to delete this committee member?")}
            {selectedMember && (
              <span className="font-medium"> {selectedMember.userId?.fullName || t("This member")}</span>
            )}
            ?
          </p>
          <p className="text-red-500 mb-4">{t("This action cannot be undone.")}</p>
