...
const StudentManagement = () => {
    ...
    try {
      ...
      if (result.data && result.data.students) {
        result.data.students.forEach((student) => {
          console.log(`Student ${student.name} department:`, student.department_id)
        })
      }
  ...
  const columns = [
            ...
            {row.profile_image ? (
              <img
                ...
                onError={(e) => {
                  ...
                  e.target.src = `/placeholder.svg?height=32&width=32&text=${row.name.charAt(0)}`
                }}
              />
            ) : (
              <img
                src={`/placeholder.svg?height=32&width=32&text=${row.name.charAt(0)}`}
                alt={row.name}
                className="w-full h-full object-cover"
              />
            )}
    ...
    { header: "Phone", accessor: "phone" },
    {
      ...
      render: (row) => {
        ...
        return <span>{typeof deptName === "string" ? deptName : "Unknown"}</span>
      },
    },
    {
      ...
      if (result.status === "success") {
        ...
        if (result.data && result.data.student) {
          ...
          const newStudent = {
            ...
            department_id:
              result.data.student.department_id && typeof result.data.student.department_id === "object"
  ...
  const handleEditStudent = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:4400/api/v1/students/${currentStudent._id}`, {
        method: "PATCH",
        headers: createAuthHeaders(false), // Don't include Content-Type for FormData
        body: studentFormData,
      })
      ...
      if (result.status === "success") {
        ...
        if (result.data && result.data.student) {
          setStudents((prevStudents) =>
            prevStudents.map((student) => (student._id === currentStudent._id ? result.data.student : student)),
          )
        } else {
  ...
  const handleDeleteStudent = async (student) => {
    if (window.confirm(`Are you sure you want to delete ${student.name}?`)) {
      try {
        ...
        const response = await fetch(`http://localhost:4400/api/v1/students/${student._id}`, {
          method: "DELETE",
          headers: createAuthHeaders(),
        })
  ...
  const openEditModal = (student) => {
    ...
    if (student.profile_image) {
      setImagePreview(`http://localhost:4400/public/img/students/${student.profile_image}`);
    } else {
      setImagePreview(`/placeholder.svg?height=128&width=128&text=${student.name.charAt(0)}`);
    }
    ...
    setFormData({
      ...student,
      department_id: typeof student.department_id === "object"
    ...
    <div className="space-y-6">
      ...
      <Modal isOpen={isAddModalOpen} onClose={() => setIsAddModalOpen(false)} title="Add New Student">
        ...
        >
          ...
          <h3 className="font-medium text-gray-800 mt-4 mb-2">Address Information</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <FormField
              ...
              onChange={handleInputChange}
            />
            <FormField label="City" name="address.city" value={formData.address.city} onChange={handleInputChange} />
            <FormField
              label="Country"
              ...
              <div className="md:w-1/3">
                <div className="bg-gray-100 rounded-lg p-4 flex flex-col items-center">
                  <div className="w-32 h-32 rounded-full bg-gray-200 overflow-hidden mb-4">
                    ...
                    <img
                      src={`/placeholder.svg?height=128&width=128&text=${currentStudent.name.charAt(0)}`}
                      alt={currentStudent.name}
                      className="w-full h-full object-cover"
                    />
                  </div>
                  <h3 className="text-xl font-bold text-gray-800">{currentStudent.name}</h3>
                  <p className="text-sm text-gray-500 mb-2">{currentStudent.student_id_number}</p>
                  <span
                    className={`px-2 py-1 rounded-full text-xs ${
                      currentStudent.status === "active" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                    }`}
                  >
                    {currentStudent.status && typeof currentStudent.status === "string"
              ...
              <div className="md:w-2/3">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-gray-800">{formatDate(currentStudent.date_of_birth)}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="text-gray-800">{currentStudent.enrollment_year}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-gray-500">Department</p>
                    <p className="text-gray-800">
                      {typeof getDepartmentNameById(currentStudent.department_id) === "string"
                ...
                <h3 className="text-lg font-medium text-gray-800 mt-6 mb-4">Address</h3>
                <div className="bg-gray-50 p-4 rounded-md">
                  <p className="text-gray-800">
                    {currentStudent.address && typeof currentStudent.address === "object" ? (
                      <>
                        {currentStudent.address.street || ""}, {currentStudent.address.city || ""},{" "}
